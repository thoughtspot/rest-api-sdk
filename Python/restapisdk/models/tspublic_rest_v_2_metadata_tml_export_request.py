...
class TspublicRestV2MetadataTmlExportRequest(object):
    ...
    Attributes:
        ...
        format_type (FormatTypeEnum): The format in which to export the
            objects
        export_associated (ExportAssociatedEnum): Specifies if you would like
            to export the associated objects. To export the objects associated
            with the objects specified in id, set the value to true. When set
            to true, the API exports any underlying worksheets, tables, or
            views for a given object.   By default, the API does not export
            these underlying objects
