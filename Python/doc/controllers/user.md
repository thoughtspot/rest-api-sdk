...
| `org_id` | `int` | Query, Optional | This is applicable only if organization feature is enabled in the cluster.<br><br>Unique identifier of the organization from which the user would be deleted. If no value is provided, the organization associated with the login session is considered. |
