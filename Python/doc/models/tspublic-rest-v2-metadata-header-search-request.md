...
| `batch_size` | `int` | Optional | The number of records that should be included in the response starting from offset position. If no input is provided then first page is included in the response.<br>**Default**: `-1` |
...
| `mtype` | [`Type3Enum`](../../doc/models/type-3-enum.md) | Required | Type of the metadata object being searched. |
...
| `auto_created` | `bool` | Optional | A flag to indicate whether to list only the auto created objects. When no value is provided as input then all objects are returned. |
