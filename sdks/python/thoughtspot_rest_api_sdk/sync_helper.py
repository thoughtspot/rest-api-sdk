...
"""
    ThoughtSpot REST API SDK — synchronous execution helper.
"""  # noqa: E501
...
def _get_sync_loop() -> asyncio.AbstractEventLoop:
    ...
    """
    global _loop  # pylint: disable=global-statement  # module-level lazy singleton
    if _loop is None:
        with _loop_lock:
            if _loop is None:
                ...
                thread = threading.Thread(
                    ...
                    name="thoughtspot_rest_api_sdk-sync-runner",
                    daemon=True,
                )
                thread.start()
                _loop = loop
