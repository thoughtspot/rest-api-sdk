...
import sys
from typing import Any, Awaitable, Callable, ClassVar, Dict, List, Literal, Optional, TypedDict, Union
...
class Configuration:
    ...
    def __init__(
        ...
        access_token: Optional[Union[str, Callable[[], str], Callable[[], Awaitable[str]]]]=None,
        ...
        *,
        ...
        retry_statuses: Optional[Any] = None,
        retry_methods: Optional[Any] = None,
    ) -> None:
        """Constructor
        """
        self._base_path = "https://localhost:443" if host is None else host
        """Default Base url
        """
        ...
        """Logging Settings
        """
        self.logger["package_logger"] = logging.getLogger("thoughtspot_rest_api_sdk")
        ...
        # CUSTOM (ThoughtSpot): per-phase default request timeouts (seconds) +
        # default headers, configurable on the Configuration itself. `timeout`
        # (a float, or an httpx.Timeout) overrides the per-phase fields. The
        # timeouts are read per request by rest.py
        # (RESTClientObject._resolve_default_timeout); default_headers are applied
        # at ApiClient construction. A per-call `_request_timeout` / `_headers`
        # still overrides these.
        self.timeout = timeout
        """Default request timeout: a float (seconds, all phases) or an httpx.Timeout.
        Overrides connect/read/write/pool_timeout when set."""
        self.connect_timeout = connect_timeout
        """Default connect timeout in seconds (unset → 300s)"""
        self.read_timeout = read_timeout
        """Default read timeout in seconds (unset → 300s)"""
        self.write_timeout = write_timeout
        """Default write timeout in seconds (unset → 300s)"""
        self.pool_timeout = pool_timeout
        """Default pool-acquisition timeout in seconds (unset → 300s)"""
        self.default_headers = default_headers if default_headers is not None else {}
        """Headers added to every request (applied via ApiClient)."""

        # CUSTOM (ThoughtSpot): retry policy. The existing `retries` field is
        # repurposed as the max number of retry attempts — 0/None disables
        # retrying (the default). rest.py retries `retry_statuses` + network /
        # timeout errors with exponential backoff + jitter. POST is retried too
        # (ThoughtSpot uses POST for reads); narrow with `retry_methods`.
        self.retry_backoff_factor = retry_backoff_factor
        """Base seconds for exponential backoff between retries"""
        self.retry_max_backoff = retry_max_backoff
        """Maximum seconds to sleep for a single retry"""
        self.retry_statuses = retry_statuses if retry_statuses is not None else {429, 502, 503, 504}
        """HTTP status codes that trigger a retry (when retries > 0)"""
        self.retry_methods = retry_methods
        """HTTP methods eligible for retry; None = all methods"""
        ...
        return [
            {
                ...
                'variables': {
                    'base-url': {
                        'description': "No description provided",
                        'default_value': "https://localhost:443",
                        'enum_values': []
                        }
                    }
            }
