...
public class VariableUpdateScopeInput implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(orgIdentifier, principalType, principalIdentifier, modelIdentifier, priority);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VariableUpdateScopeInput.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VariableUpdateScopeInput` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
