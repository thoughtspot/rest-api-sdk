...
public class UpdateMetadataHeaderRequest {
  ...
  public UpdateMetadataHeaderRequest headersUpdate(List<HeaderUpdateInput> headersUpdate) {
  ...
  **/
  ...
  public void setHeadersUpdate(List<HeaderUpdateInput> headersUpdate) {
    this.headersUpdate = headersUpdate;
  }


  public UpdateMetadataHeaderRequest orgIdentifier(String orgIdentifier) {
  ...
  * @throws IOException if the JSON Object is invalid with respect to UpdateMetadataHeaderRequest
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!UpdateMetadataHeaderRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpdateMetadataHeaderRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpdateMetadataHeaderRequest` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("headers_update") != null && !jsonObj.get("headers_update").isJsonNull()) {
        JsonArray jsonArrayheadersUpdate = jsonObj.getAsJsonArray("headers_update");
        if (jsonArrayheadersUpdate != null) {
          ...
          // validate the optional field `headers_update` (array)
          for (int i = 0; i < jsonArrayheadersUpdate.size(); i++) {
            HeaderUpdateInput.validateJsonObject(jsonArrayheadersUpdate.get(i).getAsJsonObject());
          };
        }
      }
