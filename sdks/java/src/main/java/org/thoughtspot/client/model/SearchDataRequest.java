...
public class SearchDataRequest implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(queryString, logicalTableIdentifier, dataFormat, recordOffset, recordSize, runtimeFilter, runtimeSort, runtimeParamOverride);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SearchDataRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SearchDataRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
