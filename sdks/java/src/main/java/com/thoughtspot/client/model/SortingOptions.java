...
public class SortingOptions implements Serializable {
  ...
  public SortingOptions fieldName(@javax.annotation.Nullable String fieldName) {
  ...
  public SortingOptions order(@javax.annotation.Nullable String order) {
    this.order = order;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to SortingOptions
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SortingOptions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SortingOptions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SortingOptions` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
