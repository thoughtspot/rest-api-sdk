...
public class ParameterizeMetadataRequest implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(metadataType, metadataIdentifier, fieldType, fieldName, variableIdentifier);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ParameterizeMetadataRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ParameterizeMetadataRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
