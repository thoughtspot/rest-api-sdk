...
public class FetchLiveboardDataRequest implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(metadataIdentifier, visualizationIdentifiers, transientContent, dataFormat, recordOffset, recordSize, runtimeFilter, runtimeSort, runtimeParamOverride);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FetchLiveboardDataRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FetchLiveboardDataRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
