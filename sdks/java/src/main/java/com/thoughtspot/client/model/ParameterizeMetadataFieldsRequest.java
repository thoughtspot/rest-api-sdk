...
public class ParameterizeMetadataFieldsRequest implements Serializable {
    ...
    public int hashCode() {
        return Objects.hash(
                metadataType, metadataIdentifier, fieldType, fieldNames, variableIdentifier);
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!ParameterizeMetadataFieldsRequest.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                    + " `ParameterizeMetadataFieldsRequest` properties. JSON: %s",
