...
public class GenerateCSVRequest implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(startDate, endDate, calendarType, monthOffset, startDayOfWeek, quarterNamePrefix, yearNamePrefix);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GenerateCSVRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GenerateCSVRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
