...
public class ChannelHistoryJob implements Serializable {
    ...
    @Override
    public int hashCode() {
        return Objects.hash(id, status, creationTimeInMillis, detail, tryCount);
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!ChannelHistoryJob.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `ChannelHistoryJob` properties. JSON: %s",
