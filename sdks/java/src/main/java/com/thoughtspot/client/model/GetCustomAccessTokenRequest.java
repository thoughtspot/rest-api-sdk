...
public class GetCustomAccessTokenRequest implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(username, password, secretKey, validityTimeInSec, orgIdentifier, persistOption, filterRules, parameterValues, variableValues, objects, email, displayName, groups, autoCreate);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GetCustomAccessTokenRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetCustomAccessTokenRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
