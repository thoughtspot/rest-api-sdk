...
public class SearchCalendarsRequest implements Serializable {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(connectionIdentifier, namePattern, recordOffset, recordSize, sortOptions);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SearchCalendarsRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SearchCalendarsRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
