...
import com.thoughtspot.client.model.CALLBACKInput;
import com.thoughtspot.client.model.URLInput;
...
public class ActionDetailsInput implements Serializable {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ActionDetailsInput.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ActionDetailsInput.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ActionDetailsInput` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
